...
**The web build runs, and a full run has been playtested in a browser.** Three of the four things nothing else could test are confirmed: **RMB places the rally point** with no context-menu hijack (Godot's default web shell handles this — the suppression worried about since Phase 1 needed no work), **LMB casts the Barrage**, so the canvas takes focus, and **no perceptible lag** under a real swarm in WASM. Browser audio through the click-to-start gate is the one still to confirm.
